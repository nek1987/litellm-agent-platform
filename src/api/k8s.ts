/**
 * Kubernetes sandbox lifecycle. Mirrors src/api/fargate.ts so the routing
 * layer doesn't care which backend is in use — see src/api/sandbox.ts for
 * the dispatcher.
 *
 * Backend model:
...
async function buildVaultEnv(opts: RunTaskOpts): Promise<Array<{ name: string; value: string }>> {
  ...
  // src/api/auth/agent-token.ts for the format, and the /agent-auth/refresh
...
// Re-export so existing imports (`import { VaultInterception } from "@/api/k8s"`)
// keep working. The canonical definitions live in `@/shared/vault-types`.
