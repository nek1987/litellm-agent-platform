 ...
 * `src/api/integrations/core/origin.ts`. Today only Slack supplies a
...
//
// Type definitions are in `src/shared/vault-types.ts` so the server-side k8s
// helper imports the same shapes. The import + re-export at the top of this
// file keeps callers using `import { VaultInterception } from "@/ui/lib/api"`.
...
// SessionLogEvent in src/api/sessionStore.ts.
 ...
 * `MessageAttachment` shape — see `src/api/types.ts`. Images today; the
